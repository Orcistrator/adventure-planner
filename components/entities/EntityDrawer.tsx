...
function Divider({ label }: { label?: string }) {
  return (
    <div className="my-3 flex items-center gap-3">
      {label && (
        <span className="shrink-0 text-xs font-bold text-stone-950 uppercase">
          {label}
        </span>
      )}
        ...
        <div key={ti}>
          ...
          <div className="overflow-hidden rounded-lg border border-stone-200 text-xs">
            <div className="grid grid-cols-[56px_1fr] border-b border-stone-200 bg-stone-50">
              <div className="border-r border-stone-200 px-3 py-1.5 text-[10px] font-bold tracking-wider text-stone-400 uppercase">
                Roll
              </div>
            ...
            {table.rows.map((row, ri) => (
              <div
                key={ri}
                className="grid grid-cols-[56px_1fr] border-b border-stone-100 last:border-b-0"
              >
                <div className="border-r border-stone-100 px-3 py-2 text-stone-400">
                  {row.roll}
                </div>
        ...
        <div className="mb-3 flex flex-wrap gap-x-5 gap-y-1">
          {s?.ac != null && (
            <div className="flex items-center gap-1 text-xs">
              ...
              <span className="font-bold text-stone-800">AC</span>
          ...
          {s?.hp != null && (
            <div className="flex items-center gap-1 text-xs">
              ...
              <span className="font-bold text-stone-800">HP</span>
              <span className="text-stone-600">
                {s.hp}
          ...
          {s?.speed && (
            <div className="flex items-center gap-1 text-xs">
              <Footprints size={11} className="text-stone-400" />
              <span className="font-bold text-stone-800">Speed</span>
              <span className="text-stone-600">{s.speed}</span>
            </div>
          )}
          {s?.initiative != null && (
            <div className="flex items-center gap-1 text-xs">
              <Zap size={11} className="text-amber-400" />
              <span className="font-bold text-stone-800">Initiative</span>
              <span className="text-stone-600">
          ...
          (entity.personality ||
            ...
            <div>
              ...
              <div className="flex flex-col gap-1">
                ...
                {entity.ideals && (
                  <StatRow label="Ideals" value={entity.ideals} />
                )}
        ...
        <>
          ...
          {(entity.legendaryActions?.length ||
            ...
            <div>
              ...
              {entity.legendaryActionsDescription && (
                <p className="mb-2 text-xs text-stone-500 italic">
                  {entity.legendaryActionsDescription}
                </p>
              )}
              {entity.legendaryActions?.map((e, i) => (
                <p
                  key={i}
                  className="mb-1 text-xs leading-relaxed text-stone-800"
                >
                  <span className="font-bold italic">{e.name}. </span>
                  {e.description}
                </p>
      ...
      {entity && (
        <>
          ...
          <motion.div
            ...
            className="shrink-0 overflow-hidden rounded-lg bg-olive-50/95"
          >
            {displayEntity && (
              <div className="relative">
                ...
                <div className="scrollbar-hide flex max-h-[50vh] gap-8 overflow-y-auto px-40 py-20">
                  {/* Col 1: image + badge */}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-4 flex items-center gap-3">
                      <h2 className="font-heading line-clamp-1 text-xl leading-tight text-stone-950">
                        {displayEntity.name}
                      </h2>
                      {onEditAction && (
                        <button
